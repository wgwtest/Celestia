...
CreatePlanetarySystemMenu(std::string_view parentName, const PlanetarySystem* psys)
{
    ...
    for (int i = 0; i < psys->getSystemSize(); ++i)
    {
        ...
        if (!body->getName().empty())
        {
            ...
            case BodyClassification::Asteroid:
                asteroids.push_back(std::make_pair(i, UTF8ToWideString(body->getName(true))));
                break;
            case BodyClassification::Comet:
                comets.push_back(std::make_pair(i, UTF8ToWideString(body->getName(true))));
                break;
            case BodyClassification::Invisible:
                invisibles.push_back(std::make_pair(i, UTF8ToWideString(body->getName(true))));
                break;
            case BodyClassification::Moon:
                moons.push_back(std::make_pair(i, UTF8ToWideString(body->getName(true))));
                break;
            case BodyClassification::MinorMoon:
                minorMoons.push_back(std::make_pair(i, UTF8ToWideString(body->getName())));
                break;
            case BodyClassification::Planet:
                planets.push_back(std::make_pair(i, UTF8ToWideString(body->getName(true))));
                break;
            case BodyClassification::DwarfPlanet:
                dwarfPlanets.push_back(std::make_pair(i, UTF8ToWideString(body->getName())));
                break;
            case BodyClassification::Spacecraft:
                spacecraft.push_back(std::make_pair(i, UTF8ToWideString(body->getName(true))));
                break;
            }
        }
    }
    ...
    menuNames.push_back(UTF8ToWideString(_("Asteroids")));
    objects.push_back(comets);
    menuNames.push_back(UTF8ToWideString(_("Comets")));
    objects.push_back(invisibles);
    menuNames.push_back(UTF8ToWideString(_("Invisibles")));
    objects.push_back(moons);
    menuNames.push_back(UTF8ToWideString(_("Moons")));
    objects.push_back(minorMoons);
    menuNames.push_back(UTF8ToWideString(_("Minor moons")));
    objects.push_back(planets);
    menuNames.push_back(UTF8ToWideString(_("Planets")));
    objects.push_back(dwarfPlanets);
    menuNames.push_back(UTF8ToWideString(_("Dwarf planets")));
    objects.push_back(spacecraft);

    // TRANSLATORS: translate this as plural
    menuNames.push_back(UTF8ToWideString(C_("plural", "Spacecraft")));
...
CreateAlternateSurfaceMenu(const T& surfaces)
{
    ...
    AppendMenu(menu, MF_STRING, MENU_CHOOSE_SURFACE, UTF8ToWideString(_("Normal")).c_str());
    unsigned int i = 0;
    for (const auto& surface : surfaces)
    {
        ++i;
        AppendMenu(menu, MF_STRING, MENU_CHOOSE_SURFACE + i, UTF8ToWideString(surface).c_str());
    }
...
WinContextMenuHandler::requestContextMenu(float x, float y, Selection sel)
{
    ...
    case SelectionType::Body:
        {
            ...
            AppendMenu(hMenu, MF_STRING, ID_NAVIGATION_CENTER, UTF8ToWideString(name).c_str());
            AppendMenu(hMenu, MF_SEPARATOR, 0, 0);
            AppendMenu(hMenu, MF_STRING, ID_NAVIGATION_GOTO, UTF8ToWideString(_("&Goto")).c_str());
            AppendMenu(hMenu, MF_STRING, ID_NAVIGATION_FOLLOW, UTF8ToWideString(_("&Follow")).c_str());
            AppendMenu(hMenu, MF_STRING, ID_NAVIGATION_SYNCORBIT, UTF8ToWideString(_("S&ync Orbit")).c_str());
            AppendMenu(hMenu, MF_STRING, ID_INFO, UTF8ToWideString(_("&Info")).c_str());
            HMENU refVectorMenu = CreatePopupMenu();
            AppendMenu(hMenu, MF_POPUP | MF_STRING, reinterpret_cast<UINT_PTR>(refVectorMenu), UTF8ToWideString(_("&Reference Marks")).c_str());
            AppendMenu(refVectorMenu, MF_STRING, ID_RENDER_BODY_AXES, UTF8ToWideString(_("Show Body Axes")).c_str());
            AppendMenu(refVectorMenu, MF_STRING, ID_RENDER_FRAME_AXES, UTF8ToWideString(_("Show Frame Axes")).c_str());
            AppendMenu(refVectorMenu, MF_STRING, ID_RENDER_SUN_DIRECTION, UTF8ToWideString(_("Show Sun Direction")).c_str());
            AppendMenu(refVectorMenu, MF_STRING, ID_RENDER_VELOCITY_VECTOR, UTF8ToWideString(_("Show Velocity Vector")).c_str());
            AppendMenu(refVectorMenu, MF_STRING, ID_RENDER_PLANETOGRAPHIC_GRID, UTF8ToWideString(_("Show Planetographic Grid")).c_str());
            AppendMenu(refVectorMenu, MF_STRING, ID_RENDER_TERMINATOR, UTF8ToWideString(_("Show Terminator")).c_str());
            ...
            AppendMenu(hMenu, MF_STRING, ID_SELECT_PRIMARY_BODY, UTF8ToWideString(_("Select &Primary Body")).c_str());
    ...
    case SelectionType::Star:
        {
            ...
            if (solarSys != nullptr)
            {
                ...
                if (name == "Sol")
                    AppendMenu(hMenu, MF_POPUP | MF_STRING, (UINT_PTR) planetsMenu, UTF8ToWideString(_("Orbiting Bodies")).c_str());
                else
                    AppendMenu(hMenu, MF_POPUP | MF_STRING, (UINT_PTR) planetsMenu, UTF8ToWideString(_("Planets")).c_str());
            }
        }
        break;
    ...
    if (appCore->getSimulation()->getUniverse()->isMarked(sel, 1))
        AppendMenu(hMenu, MF_STRING, ID_TOOLS_UNMARK, UTF8ToWideString(_("&Unmark")).c_str());
    else
        AppendMenu(hMenu, MF_STRING, ID_TOOLS_MARK, UTF8ToWideString(_("&Mark")).c_str());
