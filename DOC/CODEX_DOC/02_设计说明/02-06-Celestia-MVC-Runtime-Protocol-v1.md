...
## 11. Step7 transport 落地约束（2026-06-25）

Step7 已将本机长期 IPC 的第一实现固定为 live stdio pipe：

```text
launcher / ProcessSupervisor / RuntimeSession
  -> HostProcess
    -> child stdin pipe  -> celestia-*-host --serve
    <- child stdout pipe <- celestia-*-host --serve
```
...
ProcessSupervisor 可以通过 RuntimeSession 持续路由 View -> Controller -> Model -> View 的最小消息闭环。
SDL multi-process serve 主路径可以使用 --host-transport=stdio-pipe。
```
...
View 已经 DLL / ABI 插件化热插拔。
M / C / V 已经可以跨机器部署。
真实 Celestia SimulationBackend 已经完整迁入独立 Model 进程。
```
