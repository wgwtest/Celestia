# Celestia 真实场景投影与 SceneFrame 契约

日期：2026-06-27

## 1. 目标与非目标

本文定义 Step12 的 `scene.frame vNext` 契约。它的目标是把后续真实 Model Backend、真实 SceneExtractor、跨进程 View3D、Controller 交互和 Resource/DataPlane 工作约束到同一个稳定输出协议上。
