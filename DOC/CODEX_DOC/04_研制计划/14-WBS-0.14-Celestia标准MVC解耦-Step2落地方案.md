...
Step2 已按本方案在 Celestia 本仓库内完成代码落地，提交序列如下：

```text
6798067 docs: define MVC Step2 implementation plan
65d3526 fix: route ring texture access through render assets
291accd test: add MVC Step2 boundary contract
15ff8aa refactor: decouple body model from render assets
81330bd refactor: decouple star details from render assets
7f07611 refactor: split nebula semantic and render asset loading
2f46c7d refactor: inject selection geometry provider
49ee8c4 test: prove headless MVC view consumption
```

本节所在提交记录文档闭环与最终验收证据，因此不提前列入上方代码落地提交序列。

验收结论：

| 验收项 | 结果 |
| --- | --- |
| `body.cpp`、`star.cpp`、`nebula.cpp` 不再调用具体 `*RenderAssets::` | 通过 |
| 三个 Model 实现文件不再 include 具体渲染资产 / mesh / render 资源头 | 通过 |
| `star.h` 不再 forward declare 或 friend `StarRenderAssets` | 通过 |
| DSO / Nebula 语义加载与 mesh / geometry 资产加载分离 | 通过 |
| `SelectionPicker` 通过 `SelectionGeometryProvider` 消费可替换拾取资源 | 通过 |
| `SceneViewModel` 提供 headless View Adapter 快照证明 | 通过 |
| baseline + SDL 双构建、全量 `ctest`、SDL runtime smoke | 通过 |
...
SDL runtime smoke: celestia-sdl.exe entered the run loop for 6 seconds with build-mvc-sdl-rel\run-minimal and was stopped
```

注意：原计划中的 `ctest -R unit` 只作为单元验证意图描述；当前 CTest 注册粒度为 doctest case，最终验收以不带 `-R` 的全量 `ctest --output-on-failure` 为准。
...
- [x] **Step 3: 让 BodyRenderAssets 注册生命周期回调**
...
- [x] **Step 1: 定义 StarDetails 生命周期和默认类型事件**
...
- [x] **Step 2: 从 `star.h` 移除 StarRenderAssets 友元**
...
- [x] **Step 3: 用生命周期事件替代 star.cpp 中的 StarRenderAssets 调用**
...
- [x] **Step 1: 收缩 DeepSkyObject load API**
...
- [x] **Step 3: 新增 NebulaRenderAssetLoader**
...
- [x] **Step 4: DSODatabaseBuilder 编排语义加载和资产加载**
...
- [x] **Step 2: 修改 SelectionPicker 构造函数**
...
- [x] **Step 3: 在 CelestiaCore 建立默认 provider**
...
- [x] **Step 5: Commit picker provider**
...
- [x] **Step 4: Commit headless proof**
...
```text
build-mvc-baseline-rel: build passed
build-mvc-baseline-rel: 47/47 full test count passed
build-mvc-sdl-rel: build passed
build-mvc-sdl-rel: 47/47 full test count passed
```

- [x] **Step 4: SDL runtime smoke**
