...
Step8: 3D View 跨进程渲染与 GPU 资源边界
Step9: 二进制插件 ABI / DLL hot reload
...
Step6 完成后，下一阶段原本可以分两条线：

```text
候选线 A: OpenGL 3D View 进程化
...
工程收口后统一执行口径为：Step7 先做长期 IPC + ProcessSupervisor runtime 化，OpenGL 3D View 进程化顺延为 Step8。原因是必须先稳定本机三进程长期通信和进程监管，再处理窗口、OpenGL context、GPU 资源与 scene state 同步。

这两条线无论先后都必须复用 Step6 的 `RuntimeEnvelope`、lifecycle 和 shutdown 语义，不能重新发明一套消息协议。
